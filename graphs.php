<?php
    ...
    try {
      ...
      // effect
      $result = $conn-> prepare('SELECT id, effect, effectHour, voltage, ampere, timeStamp, totalStr FROM SM3 WHERE id = :smid');
      $result->execute(array('smid' => 8));
      //effecthour
      $result2 = $conn-> prepare('SELECT id, effect, effectHour, voltage, ampere, timeStamp, totalStr FROM SM3 WHERE id = :smid');
      $result2->execute(array('smid' => 8));
      //voltage
      $result3 = $conn-> prepare('SELECT id, effect, effectHour, voltage, ampere, timeStamp, totalStr FROM SM3 WHERE id = :smid');
      $result3->execute(array('smid' => 8));
      //ampere
      $result4 = $conn-> prepare('SELECT id, effect, effectHour, voltage, ampere, timeStamp, totalStr FROM SM3 WHERE id = :smid');
      $result4->execute(array('smid' => 8));
